...
class T1DSimEnvBolus(gym.Env):
    ...
    def step(self, action):
        ...
        if (action == np.zeros_like(self.action_space)).all():
            ...
            # Calculate bolus
            CHO_estimation_factor = np.random.uniform(0.7, 1.1) # Under- or overestimation by patient
            bolus_factors = np.array([
                CHO_estimation_factor*sum(self.CHO_hist)*self.t1dsimenv.sensor.sample_time / self.ICR, 
