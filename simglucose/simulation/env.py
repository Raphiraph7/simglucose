...
class T1DSimEnv(object):
    ...
    def show_history(self):
        ...
        df['BG'] = pd.Series(self.BG_hist, dtype='float64')
        df['CGM'] = pd.Series(self.CGM_hist, dtype='float64')
        df['CHO'] = pd.Series(self.CHO_hist, dtype='float64')
        df['insulin'] = pd.Series(self.insulin_hist, dtype='float64')
        df['LBGI'] = pd.Series(self.LBGI_hist, dtype='float64')
        df['HBGI'] = pd.Series(self.HBGI_hist, dtype='float64')
        df['Risk'] = pd.Series(self.risk_hist, dtype='float64')
